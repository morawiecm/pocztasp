...
namespace SIPS
{
    class Wykaz_Poprawiony_Polecone_PP_PS
    {
       ...
       // MySqlCommand cmd;
        ...
        public Wykaz_Poprawiony_Polecone_PP_PS(string t9, string t10, string t11, Connect_Mysql con)
        {
            ...
            FileInfo info = new FileInfo(@"c:\Program\Popraw_Polecone.pdf");
            if (ReadyForReading(info) == true)
            { //uruchomienie procesu tworzenia pdfa
                MySqlCommand cmd = new MySqlCommand(select1, conn.connection);
                ...
                PdfWriter pdf = PdfWriter.GetInstance(doc, new FileStream(@"c:\Program\Popraw_Polecone.pdf", FileMode.Create));
                ...
                p = new Paragraph("POCZTA SPECJALNA", bold);
                p.Alignment = Element.ALIGN_CENTER;
                doc.Add(p);
                ...
                System.Diagnostics.Process.Start(@"c:\Program\Popraw_Polecone.pdf");
            }//koniec tworzenia pdf-a
            else
            {
                MessageBox.Show("Otwarty PDF");

                FileInfo file = new FileInfo(@"c:\Program\Popraw_Polecone.pdf");
                file.Create().Close();
            }
        }//koniec public
